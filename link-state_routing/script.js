...
function initLinkStateRouting(nodes, links) {
    ...
    document.getElementById("step-description").innerText = "Step 1: Nodes calculate their link state tables based on their immediate neighbors. ";
...
function nextStepHandler() {
    if (currentStep === 1) {
        ...
        document.getElementById("step-description").innerText = "Step 2: Nodes compute their routing tables using Dijkstra's algorithm.";
        const routingTablesDiv = document.querySelector(".routing-tables");
        routingTablesDiv.innerHTML = nodeObjects.map(node => node.printRoutingTable()).join("");
        currentStep++;
...
function handleFileUpload(event) {
    ...
    reader.onload = function (e) {
        ...
        // Assuming the file contains nodes and links in two lines
        const lines = content.split('\n');
        const nodesData = lines[0].trim();  // First line for nodes
        const linksData = lines[1].trim();  // Second line for links

        // Populate the input boxes
        document.getElementById("nodes-input").value = nodesData;
        document.getElementById("links-input").value = linksData;
    };
...
// Event listener to open the file dialog when the "Upload" button is clicked
/*document.getElementById("generate-graph-from-file").addEventListener("click", () => {
    document.getElementById("file-input").click();
});

// Event listener to handle file reading and generating the graph from the file
document.getElementById("file-input").addEventListener("change", (event) => {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            if (content) {
                // Assuming the file contains nodes and links like: "A,B,C,D\nA,B,1;A,C,2;B,D,4;C,D,3"
                const [nodesLine, linksLine] = content.split('\n');
                
                // Parse nodes and links
                nodes = nodesLine.split(',').map((id, index) => ({
                    id: id.trim(),
                    x: 100 + index * 100,
                    y: 100 + (index % 2) * 200
                }));
                
                links = linksLine.split(';').map(link => {
                    const [source, target, weight] = link.trim().split(',');
                    return { source: source.trim(), target: target.trim(), weight: +weight };
                });

                // Reset the SVG and generate the graph
                staticGraphSvg.selectAll("*").remove();
                drawGraph(staticGraphSvg, nodes, links);
                initLinkStateRouting(nodes, links);
            } else {
                alert("The file is empty or not in the correct format.");
            }
        };
