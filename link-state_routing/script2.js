...
function initLinkStateRouting(nodes, links) {
    ...
    class Node {
        ...
        printRoutingTable() {
            ...
            let html = `<h4>Routing table for ${this.name}</h4><table><tr><th>Destination</th><th>Cost</th><th>Next Hop</th></tr>`;
...
function backStepHandler() {
    if (currentStep === 2) {
        // Step 1: Go back to Link State Table view
        const routingTablesDiv = document.querySelector(".routing-tables");
        routingTablesDiv.innerHTML = previousLinkStateTables.map(table => table).join("");
